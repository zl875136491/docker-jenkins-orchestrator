...
The dependency-free control console is served by the API at [`/ui/`](http://127.0.0.1:8000/ui/). Start the local API with `uvicorn main:app --reload`, open that URL, and use the configured worker credentials to obtain a JWT. The console covers app management, paginated build history and build detail inspection, event, image, service, alert, template, and base-image endpoints; it does not add a separate API contract.
