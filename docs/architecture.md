...
Redis 仅用于 Celery broker：它不保存业务状态，也不作为 Celery result backend。MongoDB 是任务状态、事件、日志和部署数据的权威来源；worker 重启后从 MongoDB 恢复工作。开发测试可显式选择内存仓储和内存 dispatcher，但 Docker Compose 与生产配置必须使用 MongoDB 和 Redis。
...
| `build_jobs` | 唯一 `build_id`，`appid+status` | 构建状态、Jenkins 队列/构建号、部署引用和错误 |
...
1. 读取应用和构建记录，校验仓库、Compose 与模板输入；
2. 可选调用 GitLab API 验证仓库及 Git 引用；
3. 调用 Jenkins 参数化任务，传递 `APPID`、仓库、Git 引用、Compose、环境变量和目标 Harbor 镜像仓库；
...
7. 成功写入 `succeeded`，任意失败写入 `failed` 并创建告警。
...
产物缺失时，worker 不部署未知镜像，任务失败并产生可查询告警。
...
Celery beat 定期投递基础镜像同步任务。同步 worker 使用 Docker Engine 拉取源镜像、标记为 `${HARBOR_REGISTRY}/boot-images/...` 并推送 Harbor；每个版本的摘要、时间、状态和错误写入 `base_images`，绝不混入 `user_images`。
...
`docker-compose.yml` 运行 API、worker、beat、MongoDB 和 Redis。生产应使用外部受管 MongoDB/Redis、固定镜像 digest、TLS 与最小权限凭据。
